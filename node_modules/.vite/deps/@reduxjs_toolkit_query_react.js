...
function useStableQueryArgs(queryArgs) {
	const cache$1 = (0, import_react.useRef)(queryArgs);
	const copy = (0, import_react.useMemo)(() => copyWithStructuralSharing(cache$1.current, queryArgs), [queryArgs]);
	(0, import_react.useEffect)(() => {
		if (cache$1.current !== copy) cache$1.current = copy;
	}, [copy]);
	return copy;
}
function useShallowStableValue(value) {
	const cache$1 = (0, import_react.useRef)(value);
	(0, import_react.useEffect)(() => {
		if (!shallowEqual(cache$1.current, value)) cache$1.current = value;
	}, [value]);
	return shallowEqual(cache$1.current, value) ? cache$1.current : value;
}
...
function buildHooks({ api, moduleOptions: { batch: batch$1, hooks: { useDispatch: useDispatch$1, useSelector: useSelector$1, useStore: useStore$1 }, unstable__sideEffectsInRender, createSelector: createSelector$1 }, serializeQueryArgs, context }) {
	...
	function buildUseQueryState(endpointName, preSelector) {
		const useQueryState = (arg, { skip = false, selectFromResult } = {}) => {
			...
			const newLastValue = selectDefaultResult(useStore$1().getState(), lastValue.current);
	...
	function buildMutationHook(name) {
		return ({ selectFromResult, fixedCacheKey } = {}) => {
			...
			const currentState = useSelector$1((0, import_react.useMemo)(() => selectFromResult ? createSelector$1([selectDefaultResult], selectFromResult) : selectDefaultResult, [selectFromResult, selectDefaultResult]), shallowEqual);
